"""
Created on Sat November 30 20:59:55 2019
...
for fold in range(k):
    ...
    intensityTestgr=labelgrp[:,test_ix]
    ...
    #### SVD feature dimension reduction modified from AR
    UA, sa, VAT = np.linalg.svd(YTD)
    da = UA.shape[0]-1 
    Ua = UA[:,0:da+1]
    Ub = protTrain 
    SA = np.diag(sa)
    ...
    rnaTestReconstruct = YTD @ VaT.T @ np.linalg.pinv(Sa) 
    protTestReconstruct = protTest

    # make input to nn
    rnaNum=Ua.shape[0] 
    protTrainIN=Ub.repeat(rnaNum, axis=0) 
    protTestIN=protTestReconstruct.repeat(rnaNum, axis=0) 
    similarityTrainIN=YTY.reshape((-1,1), order='F') 
    protTrainNum=protTrain.shape[0]
    protTestNum=protTest.shape[0]
    rnaTrainIN=np.tile(Ua,(protTrainNum,1))  
    rnaTestIN=np.tile(rnaTestReconstruct,(protTestNum,1)) 
